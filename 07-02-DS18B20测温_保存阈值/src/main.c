...
void main()
{
	...
	while (1)
	{
		...
		Delay_ms(10);											  // 保持一段时间,否则有闪屏现象.
		KeyNum = Key_Captured();
		/*阈值判断及显示*/
		if (KeyNum)
		{
			DS18B02_SetThreshold(KeyNum);	   // 根据按键值调整阈值 TH 和 TL
			AT24C02_WriteByte(0, THigh);	   // 保存最高温阈值到数据寄存器
			Delay_ms(5);					   // 保证有足够的写入时间
			AT24C02_WriteByte(1, TLow);		   // 保存最低温阈值到数据寄存器 At24C02中 保存
			Delay_ms(5);					   // 保证有足够的写入时间
